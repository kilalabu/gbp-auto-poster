...
rg "^summary:" .claude/agent-memory-local/ --no-ignore --hidden

# 3. Search summaries for keyword
rg "^summary:.*keyword" .claude/agent-memory-local/ --no-ignore --hidden -i

# 4. Search by tag
rg "^tags:.*keyword" .claude/agent-memory-local/ --no-ignore --hidden -i

# 5. Full-text search (when summary search isn't enough)
rg "keyword" .claude/agent-memory-local/ --no-ignore --hidden -i
...
```bash
mkdir -p .claude/agent-memory-local/category-name/
# Note: Check if file exists before writing to avoid accidental overwrites
cat > .claude/agent-memory-local/category-name/filename.md << 'EOF'
---
...
- **Delete**: Remove memories that are no longer relevant
  ```bash
  trash .claude/agent-memory-local/category-name/filename.md
  # Remove empty category folders
  rmdir .claude/agent-memory-local/category-name/ 2>/dev/null || true
  ```
